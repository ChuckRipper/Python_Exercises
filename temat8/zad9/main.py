...
def obserwuj(func):
    def wrapper(*args, **kwargs):
        result = func(*args, **kwargs)
        obserwacje = obserwatorzy.get(func.__name__, {})
        komunikat = f"Funkcja {func.__name__} została wywołana o czasie {time.time()} i życzy smacznej kawusi"
        
        if obserwacje.get("anetke"):
            informuj_anetke(komunikat)
        if obserwacje.get("beatke"):
            informuj_beatke(komunikat)
        if obserwacje.get("czeske"):
            informuj_czeske(komunikat)
        if obserwacje.get("danke"):
            informuj_danke(komunikat)
        if obserwacje.get("elke"):
            informuj_elke(komunikat)
        return result
    return wrapper
